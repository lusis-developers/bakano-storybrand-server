...
export async function generateSetupPasswordEmail(
  name: string,
  setupLink: string
): Promise<string> {
  const HtmlEmail = `
            ...
            <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color: ${Colors.WHITE}; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.05); margin-top: 40px;">
              ...
              <tr>
                <td align="center" style="padding: 20px 40px 0 40px;">
                  <h1 style="margin: 0; font-size: 24px; color: ${Colors.BAKANO_PINK};">Crea tu contraseña</h1>
                </td>
              </tr>
              <tr>
                <td style="padding: 20px 40px 40px 40px;">
                  <p style="font-size: 16px; color: ${Colors.BAKANO_DARK};">Hola <strong>${name}</strong>,</p>
                  <p style="font-size: 16px; color: ${Colors.BAKANO_DARK};">Por favor crea tu contraseña para completar la configuración de tu cuenta.</p>
                  <div style="text-align: center; margin: 30px 0;">
                    <a href="${setupLink}" style="background-color: ${Colors.BAKANO_PINK}; color: ${Colors.WHITE}; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; display: inline-block;">Crear contraseña</a>
                  </div>
                  <p style="font-size: 14px; color: ${Colors.TEXT_PLACEHOLDER};">Si el botón no funciona, utiliza este enlace: ${setupLink}</p>
                </td>
              </tr>
              <tr style="background-color: ${Colors.BAKANO_DARK};">
