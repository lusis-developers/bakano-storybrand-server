...
export async function generateTeamRevokedEmail(
  name: string,
  businessName: string
): Promise<string> {
  const HtmlEmail = `
            ...
            <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color: ${Colors.WHITE}; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.05); margin-top: 40px;">
              ...
              <tr>
                <td align="center" style="padding: 20px 40px 0 40px;">
                  <h1 style="margin: 0; font-size: 24px; color: ${Colors.BAKANO_PINK};">Tu acceso fue revocado</h1>
                </td>
              </tr>
              <tr>
                <td style="padding: 20px 40px 40px 40px;">
                  <p style="font-size: 16px; color: ${Colors.BAKANO_DARK};">Hola <strong>${name}</strong>,</p>
                  <p style="font-size: 16px; color: ${Colors.BAKANO_DARK};">Tu membresía en <strong>${businessName}</strong> ha sido revocada.</p>
                </td>
              </tr>
              <tr style="background-color: ${Colors.BAKANO_DARK};">
